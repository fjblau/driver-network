...
async function setupDemo(addPortEntry) {
    ...
    //create 2 containers
    const container1 = factory.newResource(NS, 'ContainerForPickup', 'C001')
    const container2 = factory.newResource(NS, 'ContainerForPickup', 'C002')
    container1.containerStatus='Ready for Pickup'
    container2.containerStatus='Ready for Pickup'
    const containerRegistry = await getAssetRegistry(NS + '.ContainerForPickup')
    ...
    pec1.container = factory.newRelationship(NS, 'ContainerForPickup', 'C001')
    pec.port = factory.newRelationship(NS, 'Port', 'P001')
...
/**
 * add a Port Entry
 * @param {org.inet.addPortEntry} addPortEntry
 * @transaction
 */

async function addPortEntry(addPortEntry) {
    const factory = getFactory();
    const NS = 'org.inet';

    const portEntry = factory.newResource(NS, 'PortEntry', addPortEntry.portEntryId);
    portEntry.portEntryCertificate = factory.newRelationship(NS, 'PortEntryCertificate', addPortEntry.portEntryCertificate);
    portEntry.pickupAuth = factory.newRelationship(NS, 'PickupAuth', addPortEntry.pickupAuth);

    const portEntryRegistry = await getAssetRegistry(NS + '.PortEntry');
    await portEntryRegistry.addAll([portEntry]);
}