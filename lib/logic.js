...
/**
 * add a Port Entry
 * @param {org.inet.addPortEntryEvent} addPortEntryEvent
 * @transaction
 */

async function addPortEntryEvent(addPortEntryEvent) {
    const factory = getFactory();
    const NS = 'org.inet';

    const portEntry = factory.newResource(NS, 'PortEntryEvent', addPortEntryEvent.portEntryEventId);
    portEntry.portEntryCertificate = factory.newRelationship(NS, 'PortEntryCertificate', addPortEntryEvent.portEntryCertificate);
    const portEntryEventRegistry = await getAssetRegistry(NS + '.PortEntryEvent');
    await portEntryEventRegistry.addAll([portEntryEvent]);
}